...
public class CameraManager : MonoBehaviour {
    ...
    private void FixedUpdate()
    {
        if (Input.touchCount > 0)
        {
            ...
            {
                float z = transform.localPosition.z - 10 * ((Mathf.Cos(45) * Input.GetTouch(0).deltaPosition.y) / Screen.height - (Mathf.Sin(45) * Input.GetTouch(0).deltaPosition.x) / Screen.width);
                float x = transform.localPosition.x - 10 * ((Mathf.Cos(45) * Input.GetTouch(0).deltaPosition.x) / Screen.width + (Mathf.Sin(45) * Input.GetTouch(0).deltaPosition.y) / Screen.height);
